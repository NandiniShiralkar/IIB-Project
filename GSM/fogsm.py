...
class FoGSMVAE(FoGSMModel):
    ...
    def build_Sigma(self, chol_flat):
        """
        ...
        Sigma = torch.zeros(num_g_points,g_dim, g_dim)
        for i in range(num_g_points):
            ...
            Sigma[i] = torch.mm(L, L.t())

        return Sigma

    def reparameterise(self,mean, chol_flat = None, log_var=None, num_samples=100):
        """
        ...
        if log_var is not None:
            ...
            self.Sigma = torch.zeros(num_g_points, g_dim, g_dim)
            for i in range(num_g_points):
                mean_g = mean[i]
                chol_flat_g = chol_flat[i]
        
                # Convert chol_flat into a lower triangular matrix for the i-th GP
                L = torch.zeros((g_dim, g_dim))
                tril_indices = torch.tril_indices(row=g_dim, col=g_dim)
                L[tril_indices[0], tril_indices[1]] = chol_flat_g
                self.Sigma[i] = torch.mm(L, L.t())  # Sigma = LL^T
    ...
    def forward(self, I):
        ...
        g_samples = self.reparameterise(g_mean, chol_flat=g_chol_flat,num_samples=1000)
